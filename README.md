...
- Защита осмотического фильтра: при подмесе ГВС в ХВС реле перекрывает подачу воды на фильтр, обратно открывает по гистерезису
...
- **PIN_FILTER_RELAY** — пин реле клапана фильтра (D1=5). Задаётся в `src/FilterGuard.h`, а не в `secrets.h`: тот в `.gitignore`
...
- **Osmosis Filter Guard** — включение, пороги закрытия и открытия (°C), полярность модуля реле, каналы уведомления (Home Assistant, почта, `/metrics`)
- **Send scheduled reports** — общий выключатель плановых писем: SMTP остаётся настроенным, Test Email и алерты фильтра продолжают работать
- **Счётчики** — текущие показания (м³) и цена импульса в литрах. Дробная часть принимается и через точку, и через запятую; нечисловое значение не сохраняется, а подсвечивается ошибкой
- Незаконченный ввод не затирается: пока форма изменена, устройство не переписывает поля своими значениями (см. [docs/pitfalls.md](docs/pitfalls.md), раздел 27)
