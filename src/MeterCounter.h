 ...
 *
 * ПОЧЕМУ evaluate() ВЫЗЫВАЕТСЯ ВНУТРИ РАЗБОРА ОЧЕРЕДИ
 *
 * Импульс может целиком уложиться в паузу между двумя process():
 * loop() занят отправкой почты, подключением к MQTT или чтением шины
 * 1-Wire, а вода в это время течёт. Тогда в очереди оказываются сразу
 * оба фронта — замыкающий и размыкающий.
 *
 * Если разобрать их подряд, не проверяя зрелость кандидата между ними,
 * второй фронт вернёт уровень к подтверждённому, и feed() спишет всю
 * завершившуюся фазу в дребезг. Импульс пропадал молча, а счётчик
 * дребезга подрастал:
 *
 *   t=0     FALLING -> кандидат «замкнуто»
 *   t=120мс RISING  -> уровень снова HIGH == подтверждённому
 *                      -> кандидат снят как дребезг, импульса НЕТ
 *
 * Поэтому перед каждым feed() автомат проверяется на момент фронта:
 * кандидат, проживший своё окно, подтверждается до того, как придёт
 * обратный фронт.
 *
 * ЗАЛИПШИЙ ГЕРКОН
 *
 ...
 * ГРАНИЦЫ
 *
 * Разбор не зависит от длительности импульса — кроме самих порогов
 * антидребезга:
 *
 *   - замыкание короче debounceClosedMs         -> дребезг, не импульс
 *   - пауза короче debounceOpenMs               -> замыкания сливаются
 *     (отсюда потолок ~18 имп/с при значениях по умолчанию)
 *   - сверху длительности не ограничены ничем
 *   - переполнение micros() безразлично: сравнения идут на разностях
 *
 * Настоящий предел — не длительность, а ЧИСЛО ФРОНТОВ за одну паузу
 * loop(): очередь вмещает METER_EVENT_QUEUE-1 событий, то есть 15
 * чистых импульсов. Всё сверх теряется, и это видно в queueOverflow().
 *
 * Единственная зависимость от длительности — в диагностике: после
 * замыкания длиннее ~49 суток возраст насыщается, длительность фазы
 * известна лишь как «не меньше», и в min/max она не идёт. Сам импульс
 * при этом считается как обычно.
 */

#define METER_EVENT_QUEUE           32
...
private:
  ...
  // nowUs — момент, НА КОТОРЫЙ проверяется зрелость кандидата. При
  // разборе очереди это время следующего фронта, то есть прошлое; сам
  // же возраст состояния всегда считаем от реального micros().
  void evaluate(uint32_t nowUs) {
    ...
    // Сколько прошло от физического фронта до этого мгновения. Раньше
    // здесь стояло needUs — верно только когда кандидат подтверждается
    // ровно на границе окна. При разборе очереди фронт может оказаться
    // на сотни миллисекунд в прошлом, и длительность фазы уехала бы.
    //
    // Сверху lag не ограничиваем: долгая пауза loop() — это честная
    // задержка, и возраст состояния должен её учесть. Снизу он не может
    // быть меньше окна подтверждения, это инвариант проверки выше.
    uint32_t lagUs = (uint32_t)(micros() - _candSinceUs);
    if (lagUs < needUs) lagUs = needUs;
    uint32_t lagMs = lagUs / 1000;

    // Возраст насыщается на ~49 сутках (см. stateAgeMs). После такого
    // простоя длительность фазы известна только как «не меньше», и в
    // min/max её пускать нельзя — иначе максимум навсегда застрянет на
    // 4.29e9, а столбец Closed ms перестанет годиться для подбора
    // порогов. Сам импульс при этом считается как обычно.
    uint32_t age = stateAgeMs();
    bool phaseKnown = (age != UINT32_MAX);
    uint32_t phaseMs = (age > lagMs) ? (age - lagMs) : 0;
...
public:
  ...
  bool process() {
    ...
    while (true) {
      ...
      interrupts();
      if (!have) break;
      // Кандидат мог созреть ЕЩЁ ДО этого фронта. Проверяем его на
      // момент фронта: иначе feed() увидит возврат к подтверждённому
      // уровню и спишет завершившуюся фазу в дребезг.
      evaluate(e.tUs);
      feed(e.tUs, e.level);
    }
