...
| `Error Uploading` на `uploadfs`, обрыв на первых процентах | Образ LittleFS — все 2 МБ раздела, и первая попытка срывается заметно чаще, чем на прошивке | Повторить `uploadfs`. **Обязательно проверить результат** — см. ниже |

### Оборванный `uploadfs` оставляет файловую систему битой

Прерванная заливка записывает образ частично, и устройство продолжает отдавать `index.html`, у которого начало новое, а хвост — нули. Внешне страница «просто не работает», размер в заголовке при этом правильный, и на глаз подмена не видна.

После каждой заливки FS сверяйте контрольную сумму того, что реально отдаёт плата:

```bash
curl --noproxy '*' -s http://<ip>/ -o /tmp/served.html
md5sum /tmp/served.html data/index.html    # суммы должны совпасть
```

Флаг `--noproxy '*'` обязателен, если в системе задан прокси: на адреса `192.168.x.x` он отвечает 503.
