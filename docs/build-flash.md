...
> **Важно:** `pio run --target upload` **не затирает** EEPROM (настройки WiFi, SMTP, калибровку). EEPROM лежит по смещению `0x3FB000`, прошивка пишется в `0x000000..0x100000` — они не пересекаются.
>
> Причины, по которым настройки всё же могут пропасть, перечислены в [pitfalls.md, раздел 11](pitfalls.md).
...
5. **После OTA подтвердите прошивку** — иначе устройство один раз перезагрузится через 5 минут:
   - Откройте `http://192.168.88.87/confirm` в браузере
   - Или нажмите **Confirm** на OTA-баннере в веб-интерфейсе
   - Или выполните `confirm` в Telnet

   Отката к старой прошивке при этом не происходит — его на ESP8266 не существует (см. [modules/ota.md](modules/ota.md)). Подтверждение — это признак того, что новая прошивка дожила до `loop()`.
