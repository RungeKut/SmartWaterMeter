...
Отправляет их **не колбэк подключения, а `loop()`**: `WS_EVT_CONNECT` только ставит отметку через `requestSnapshot()`, а `handleSnapshots()` шлёт по одному сообщению за проход, пропуская шаг при занятой очереди клиента. Сборка JSON внутри колбэка ESPAsyncTCP роняла плату примерно в 15% подключений — там меньше стека, чем в `loop()`.
