...
| `getExpectedAddr(index)` | Адрес, закреплённый за каналом (даже если датчика нет на шине) |
| `hasAssignedAddr(index)` | Закреплён ли осмысленный адрес (проверка CRC — отсекает заглушки из `secrets.h`) |
...
## Признак присутствия датчика

`_found[i]` определяется тем, **читается ли канал**, а не тем, нашёлся ли он поиском по шине. Это принципиально.

Подвисающий датчик регулярно не попадает в поиск `getAddress()`, но на прямой запрос по адресу (MatchROM) отвечает нормально. Проверено на живом устройстве: при опоре на поиск канал Cold числился Missing в 92 замерах из 98, показывая при этом корректную температуру.

Порядок чтения канала:

1. Если адрес есть в текущем списке шины — берём значение из кеша `_busTemps[]`
2. Если нет — пробуем прямой запрос по адресу, с одной повторной попыткой
3. Успех → `_found = true`; неудача → счётчик `_chFails[i]`, и только после `MAX_READ_FAILURES` подряд канал становится Missing

Раньше `_found[]` выставлялся один раз в `begin()` и больше не менялся. Подключённый на ходу датчик оставался Missing до перезагрузки, а отключённый продолжал числиться Found. Теперь признак живой в обе стороны.
